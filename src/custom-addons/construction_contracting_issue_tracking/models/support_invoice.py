...
class ConstructionInvoiceLine(models.Model):
    ...
    construction_id = fields.Many2one(
        'construction.ticket',
        string='Construction Invoice Issue',
    )
    ...
    def _compute_tax_id(self):
        for line in self:
            fpos = line.construction_id.partner_id.property_account_position_id
    ...
    def product_uom_change(self):
        if not self.product_uom:
            self.price_unit = 0.0
            return
        if self.construction_id.partner_id.property_product_pricelist and self.construction_id.partner_id:
            product = self.product_id.with_context(
                lang=self.construction_id.partner_id.lang,
                partner=self.construction_id.partner_id.id,
                quantity=self.product_uom_qty,
                date_order=fields.Datetime.now,
                pricelist=self.construction_id.partner_id.property_product_pricelist.id,
