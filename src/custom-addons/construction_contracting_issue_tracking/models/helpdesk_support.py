...
#     _inherit = ['mail.thread', 'ir.needaction_mixin']
    ...
    def _write(self, vals):#this is to fix access error on stage write with other records.
        if len(vals.keys()) == 1 and 'stage_type' in vals:
            return super(ConstructionTicket, self.sudo())._write(vals)
        return super(ConstructionTicket, self)._write(vals)
    ...
    def create(self, vals):
        if vals.get('name', False):
            if vals.get('name', 'New') != 'New':
                ...
                vals['name'] = 'New'
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('construction.ticket') or 'New'
        ...
        if vals.get('team_id') and not vals.get('team_leader_id'):
            vals['team_leader_id'] = self.env['construction.team'].browse(vals.get('team_id')).leader_id.id

        # context: no_log, because subtype already handle this
        return super(ConstructionTicket, self.with_context(context, mail_create_nolog=True)).create(vals)
...
#        return super(HelpdeskSupport, self).create(vals)
    ...
    def set_to_close(self):
        stage_id = self.env['construction.stage.config'].search([('stage_type','=','closed')])
        if self.is_close != True:
            self.is_close = True
            self.close_date = fields.Datetime.now()#time.strftime('%Y-%m-%d')
            self.stage_id = stage_id.id
#            self.state = 'closed'
            template = self.env.ref('construction_contracting_issue_tracking.email_template_construction_ticket')
            template.send_mail(self.id, force_send=True)
            
    @api.one
    def set_to_reopen(self):
        stage_id = self.env['construction.stage.config'].search([('stage_type','=','work_in_progress')])
...
#            self.state = 'work_in_progress'
    ...
    def _default_stage_id(self):
        team = self.env['construction.team'].sudo()._get_default_team_id(user_id=self.env.uid)
    ...
    def _stage_find(self, team_id=False, domain=None, order='sequence'):
        ...
        return self.env['construction.stage.config'].search(search_domain, order=order, limit=1)
...
#     )
    ...
    category = fields.Selection(
        ...
        ('functional', 'Material Deficiencies'),
        ('support', 'Construction Deficiencies')],
        string='Category',
    )
    subject = fields.Char(
        string="Subject"
...
#        'support_request_id',
        'construction_ticket_request_id',
        string='Timesheets',
    )
    is_close = fields.Boolean(
        string='Is Issue Closed ?',
        track_visibility='onchange',
        default=False,
        copy=False,
    )
...
#        'support.team',
        'construction.team',
        string='Construction Team',
        default=lambda self: self.env['construction.team'].sudo()._get_default_team_id(user_id=self.env.uid),
    )
...
#         store=True,
    ...
    subject_type_id = fields.Many2one(
        'construction.type.of.subject',
        string="Type of Issue Subject",
        copy=True,
    )
...
#     )
    ...
    @api.depends('analytic_account_id',
                 'analytic_account_id.prepared_hours_ids',
                 'analytic_account_id.total_purchase_hours',
                 'analytic_account_id.total_consumed_hours',
                 'analytic_account_id.remaining_hours')
    ...
    def unlink(self):
        ...
        return super(ConstructionTicket, self).unlink()
...
#                         'price_subtotal' : line.product_id.standard_price * line.quantity,
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        return super(ConstructionTicket, self).message_new(msg_dict, custom_values=defaults)
        
class HrTimesheetSheet(models.Model):
    _inherit = 'account.analytic.line'

    construction_ticket_request_id = fields.Many2one(
        'construction.ticket',
