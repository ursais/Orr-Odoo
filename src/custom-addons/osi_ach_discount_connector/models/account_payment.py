        ...
        for payment in self:
            ...
            if payment_method:
                if payment_method.code in ('ACH-In', 'ACH-Out'):
                    ...
                    if not payment.invoice_ids.payment_mode_id:
                        payment_mode_id = self.env[
                            'account.payment.mode'].search(
                            [('payment_type', '=', payment.payment_type),
                             ('payment_method_id', '=', payment_method.id),
                             ('payment_order_ok', '=', True)], limit=1)
                        if payment_mode_id:
                            payment.invoice_ids.write(
                                {'payment_mode_id': payment_mode_id.id})
                            payment.invoice_ids.move_id.line_ids.write(
                                {'payment_mode_id': payment_mode_id.id})
                    action = payment.invoice_ids.create_account_payment_line()
                    payment.unlink()
                    return action
...
class AccountPaymentLine(models.Model):
    ...
    payment_difference_handling = fields.Selection(
        ...
        default='reconcile', string="Action", copy=False)
