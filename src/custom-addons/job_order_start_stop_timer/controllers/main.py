...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        domain = ['|', ('project_id', 'in', projects.ids), ('user_id', '=', request._uid)]
        values['job_order_count'] = Task.search_count(domain)
        print ("::::::::::::::values",values)
        return values

    @http.route(['/user/job_orders', '/user/job_orders/page/<int:page>'], type='http', auth="user", website=True)
    def portal_user_job_orders(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None, search_in='content', **kw):
        ...
        projects = request.env['project.project'].sudo().search([('user_id', '=', request._uid)])
        domain = ['|', ('project_id', 'in', projects.ids), ('user_id', '=', request._uid)]
