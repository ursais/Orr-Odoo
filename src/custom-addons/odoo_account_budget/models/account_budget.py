...
#    _name = "account.budget.post"
    ...
    company_id = fields.Many2one(
        ...
        'Company',
        required=True,
        default=lambda self: self.env['res.company']._company_default_get('account.analytic.budget.post')
        #default=lambda self: self.env['res.company']._company_default_get('account.budget.post')
    )

    def _check_account_ids(self, vals):
        # Raise an error to prevent the account.analytic.budget.post to have not specified account_ids.
...
class CrossoveredBudget(models.Model):
    _name = "account.analytic.crossovered.budget"
...
#    _name = "crossovered.budget.lines"
    _name = "account.analytic.crossovered.budget.lines"
    _description = "Budget Line"

    crossovered_budget_id = fields.Many2one(
    	'account.analytic.crossovered.budget',
    	'Budget',
    	ondelete='cascade',
    	index=True,
    	required=True
    ...
    practical_amount_stored = fields.Float(
        compute='_compute_practical_amount_store', 
    ...
    theoritical_amount_stored = fields.Float(
        compute='_compute_theoritical_amount_store', 
        string='Theoretical Amount', 
        digits=0,
        store=True
    )
    percentage = fields.Float(compute='_compute_percentage', string='Achievement')
    percentage_stored = fields.Float(compute='_compute_percentage_store', string='Achievement')
    ...
    def _compute_practical_amount(self):
        for line in self:
            ...
            if line.analytic_account_id.id:
                ...
                result = sum(analytic_line.mapped("amount"))
#                self.env.cr.execute("""
#                    SELECT SUM(amount)
...
#                result = self.env.cr.fetchone()[0] or 0.0
    ...
    @api.multi
    def _compute_practical_amount_store(self):
        for line in self:
            result = 0.0
            acc_ids = line.general_budget_id.account_ids.ids
            date_to = self.env.context.get('wizard_date_to') or line.date_to
...
#                (line.analytic_account_id.id, date_from, date_to, acc_ids,))
#                result = self.env.cr.fetchone()[0] or 0.0
            line.practical_amount_stored = result
    ...
    @api.depends('date_from',
                 ...
                 'planned_amount')
    @api.multi
    def _compute_theoritical_amount_store(self):
        today = fields.Datetime.now()
        theo_amt = 0.00
        for line in self:
            # Used for the report
                ...
                if line.paid_date:
                    ...
                    if line.date_to and line.date_from:
                        ...
                        if elapsed_timedelta.days < 0:
                            ...
                            theo_amt = line.planned_amount
            line.theoritical_amount_stored = theo_amt
    ...
    @api.multi
    def _compute_percentage_store(self):
        for line in self:
            if line.theoritical_amount != 0.00:
                line.percentage_stored = float((line.practical_amount or 0.0) / line.theoritical_amount) * 100
            else:
                line.percentage_stored = 0.00
