...
#     _inherit = ['mail.thread', 'ir.needaction_mixin']
    ...
    def create(self, vals):
        ...
        if 'commission_user_id' in vals and not vals.get('commission_user_id'):
            vals.update({'commission_user_id': self.env.uid})
...
#             raise UserError(_('Please configure an accounting sale journal for this company.'))
        ...
        invoice_vals = {
            ...
            'origin': self.name,
            'type': 'in_invoice',
            'account_id': partner.property_account_payable_id.id,#property_account_receivable_id.id,
            'partner_id': partner.id,
            'journal_id': journal_id.id,
            'currency_id': partner.property_product_pricelist.currency_id.id,#partner.property_product_pricelist.currency_id.id,
