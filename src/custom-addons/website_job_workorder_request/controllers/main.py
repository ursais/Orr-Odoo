...
class website_account(website_account):
    ...
    def show_joborder(self, **post):
        ...
        return request.render("website_job_workorder_request.job_order", values)
...
#            'job_partner_id' : partner.id,
        ...
        if post.get('job_category'):
            vale.update({
                'custom_job_category_id': int(post.get('job_category')),
            })
...
#        project_id = request.env['project.project'].sudo().search([('code', '=', post.get('project_code'))], limit=1)
    ...
    def joborder_print(self, joborder, access_token=None, **kw):
        ...
        pdf, _ = request.env.ref('website_job_workorder_request.joborder_report').sudo().render_qweb_pdf([joborder])
        pdfhttpheaders = [
