...
#     account_ids = fields.One2many('hr_timesheet_sheet.sheet.account', 'sheet_id', string='Analytic accounts', readonly=True)
    ...
    def _check_sheet_date(self, forced_user_id=False):
        for sheet in self:
            ...
            if new_user_id:
                ...
                if sheet.date_from >= sheet.date_to: #28/11/2019
                    raise ValidationError(_('Date from can not be set higher then Date to.')) #28/11/2019                    
    ...
    def name_get(self):
        ...
        return [(r['id'], _('Week ') + str(datetime.strptime(str(r['date_from']), '%Y-%m-%d').isocalendar()[1]))
...
#                return True
#                #return 'odoo_timesheet_sheet.mt_timesheet_confirmed'
#            elif 'state' in init_values and record.state == 'done':
#                #return 'odoo_timesheet_sheet.mt_timesheet_approved'
#                return True
